...
import { formatCurrencyMc, TRANCHE_CONFIGS } from '@/lib/calculations';

export function Portfolio() {
  const navigate = useNavigate();
  const { currentUser, businesses, investments } = useStore();
  ...
  const summary = useMemo(() => {
    ...
    return { totalInvested, totalPotentialProfit, totalPotentialLoss };
  }, [userInvestments]);
      ...
      <div className="max-w-[800px] mx-auto px-4 py-4">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--app-fg-muted)] uppercase tracking-wider">
              {userInvestments.length} active investment{userInvestments.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] p-4 text-center">
            <p className="text-[10px] text-[var(--app-fg-muted)] uppercase tracking-wider mb-1">Total Invested</p>
            <p className="text-lg font-bold font-mono text-[var(--app-fg)]">{formatCurrencyMc(summary.totalInvested)}</p>
          </div>
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-center">
            <p className="text-[10px] text-emerald-400 uppercase tracking-wider mb-1">Potential Gain</p>
            <p className="text-lg font-bold font-mono text-emerald-400">+{formatCurrencyMc(summary.totalPotentialProfit)}</p>
          </div>
          <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4 text-center">
            <p className="text-[10px] text-red-400 uppercase tracking-wider mb-1">Potential Risk</p>
            <p className="text-lg font-bold font-mono text-red-400">-{formatCurrencyMc(summary.totalPotentialLoss)}</p>
          </div>
        </div>
        ...
        {userInvestments.length > 0 ? (
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-[var(--app-fg)] uppercase tracking-wider">Holdings by Business</h3>
            {Object.entries(byBusiness).map(([bizId, invs]) => {
              const biz = businesses.find((b) => b.id === bizId);
              const bizInvested = invs.reduce((s, i) => s + i.depositedMillicents, 0);
              const bizProfit = invs.reduce((s, i) => s + TRANCHE_CONFIGS[i.trancheId].profitPerUnit * i.units, 0);
              return (
                <div key={bizId} className="rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs font-mono font-bold text-[var(--app-fg)]">{formatCurrencyMc(bizInvested)}</span>
                  </div>
                  ...
                  <div className="mt-3 pt-2 border-t border-[var(--card-border)] flex justify-between text-xs">
                    <span className="text-[var(--app-fg-muted)]">Potential return from {biz?.name}</span>
                    <span className="font-mono font-bold text-emerald-400">+{formatCurrencyMc(bizProfit)}</span>
                  </div>
                </div>
              );
            })}
          ...
          <div className="text-center py-12 rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)]">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-[var(--app-fg-subtle)] mx-auto mb-4 opacity-40"><path d="m22 7-8.5 8.5-5-5L2 17"/><path d="M16 7h6v6"/></svg>
            <p className="text-sm text-[var(--app-fg-subtle)] mb-1">Your portfolio is empty</p>
            <p className="text-xs text-[var(--app-fg-muted)] mb-4">Start investing in student businesses</p>
            <button
